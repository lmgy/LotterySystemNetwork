...
module.exports = function (config_filename, logger) {
	...
	if (!config_filename) {
		config_filename = 'lottery_tls.json';
	}
	...
	var package_json = require(path.join(__dirname, '../package.json'));					//get release version of lottery from package.json
	...
	// get the channel id on network for lottery
	...
	// Config Getters
	// --------------------------------------------------------------------------------
	// get the lottery owner names
	helper.getLotteryUsernames = function () {
		return getLotterysField('usernames');
	};

	// get the lottery trading company name
	helper.getCompanyName = function () {
		return getLotterysField('company');
	};

	// get the lottery server port number
	helper.getLotteryPort = function () {
		return getLotteryField('port');
	};

	// get the status of lottery previous startup
	...
	helper.getKeepAliveMs = function () {
		var sec = getLotteryField('keep_alive_secs');
		if (!sec) sec = 30;									//default to 30 seconds
		return (sec * 1000);
	};

	// safely retrieve Lottery fields
	function getLotteryField(Lottery_field) {
		try {
			if (helper.config[lottery_field]) {
				return helper.config[lottery_field];
			}
			else {
				logger.warn('"' + lottery_field + '" not found in config json: ' + helper.config_path);
				return null;
			}
		}
		catch (e) {
	...
	helper.makeUniqueId = function () {
		...
		return 'lottery-' + helper.getNetworkName() + '-' + channel + '-' + first_peer;
	};

	// build the lottery lib module options
	helper.makeLotteryLibOptions = function () {
		const channel = helper.getChannelId();
	...
	// check if lottery UI and lottery chaincode work together
	helper.errorWithVersions = function (v) {
		...
		if (v.parsed[0] !== version[0]) {					//only check the major version
			...
			logger.warn('----------------------------- Ah! -----------------------------');
			logger.warn('---------------------------------------------------------------');
			logger.error('Looks like you are using an old version of lottery chaincode...');
